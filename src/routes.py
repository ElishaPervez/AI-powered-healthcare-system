from flask import request, render_template, jsonify, flash, redirect, url_for, session, current_app as app
import numpy as np
import pandas as pd
import pickle
import bcrypt
...
def parse_list_string(s):
    if isinstance(s, list) and len(s) > 0:
        s = s[0]
    items = s.strip("[]").split(",")
    return [re.sub(r"^\\s*'|'\\s*$", "", item).strip() for item in items]
...
def helper(dis):
    ...
    if len(med) > 0:
        med = parse_list_string(med[0])
    else:
        med = []

    die = diets[diets['Disease'] == dis]['Diet'].values
    if len(die) > 0:
        die = parse_list_string(die[0])
    else:
        die = []
...
# Load data and model
sym_des = pd.read_csv("src/data/datasets/symtoms_df.csv")
precautions = pd.read_csv("src/data/datasets/precautions_df.csv")
workout = pd.read_csv("src/data/datasets/workout_df.csv")
description = pd.read_csv("src/data/datasets/description.csv")
medications = pd.read_csv('src/data/datasets/medications.csv')
diets = pd.read_csv("src/data/datasets/diets.csv")
df = pd.read_csv('src/data/datasets/Training.csv')
symptoms_list_for_template = df.columns[:-1].tolist()
symptoms_dict_for_template = {symptom: format_symptom(symptom) for symptom in symptoms_list_for_template}
svc = pickle.load(open('src/data/ml_models/svc.pkl','rb'))
...
def init_routes(app):
    @app.route("/")
    def index():
        return render_template("index.html", symptoms=symptoms_dict_for_template)
    ...
    def home():
        if request.method == 'POST':
            ...
            if not symptoms:
                ...
                return render_template('index.html', message=message, symptoms=symptoms_dict_for_template)
            ...
            try:
                ...
                return render_template('index.html', predicted_disease=predicted_disease, dis_des=dis_des,
                                       ...
                                       my_diet=rec_diet, workout=workout, symptoms=symptoms_dict_for_template)
